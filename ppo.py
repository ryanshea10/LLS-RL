        ...
        while t_so_far < total_timesteps:                                                                       # ALG STEP 2
            ...
            for _ in range(self.n_updates_per_iteration):                                                       # ALG STEP 6 & 7
                ...
                for start in range(0, step, minibatch_size):
                    ...
                    actor_loss = actor_loss - self.ent_coef * entropy_loss # Add entropy regularization
                    ...
                    nn.utils.clip_grad_norm_(self.critic.parameters(), self.max_grad_norm) # Clip gradients to prevent exploding gradients
                    self.critic_optim.step()

                    # Log losses and metrics
                    self.logger['actor_losses'].append(actor_loss.detach())
                    self.logger['critic_losses'].append(critic_loss.detach())
                    self.logger['mean_values'].append(V.mean().detach())
                    self.logger['mean_advantages'].append(A_k.mean().detach())
